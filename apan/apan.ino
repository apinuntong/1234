...
//      Serial.println(" ");
      Serial.print(((((int16_t)rx_buf[9])<<8 | (int16_t)rx_buf[10])+15)*0.2945990180032733f);
      Serial.println(" ");
      motor1.write(((((int16_t)rx_buf[9])<<8 | (int16_t)rx_buf[10])+15)*0.2945990180032733f);
      int x=(((int16_t)rx_buf[11])<<8 | (int16_t)rx_buf[12])-104;
          x=x*0.2578796561604585f;
     
      int y=(((int16_t)rx_buf[13])<<8 | (int16_t)rx_buf[14])-63;
          y=y*0.3125f;
        sovota1=(90+y)+x;
        sovota2=(90+y)-x;
        if(sovota1<0)sovota1=0;
        if(sovota1>180)sovota1=180;
        if(sovota2<0)sovota2=0;
        if(sovota2>180)sovota2=180;
        motor2.write(sovota2);
        motor3.write(sovota1);
    }
